...
You can view your IP address with the `ip address` (or, on older Linux systems, the `ifconfig -a`) command:
...
Find your IP address. Use either the `ip` command or the older, `ifconfig`, command.

>>Q1: How many bytes are in an IPv4 address?<<
=== 4
